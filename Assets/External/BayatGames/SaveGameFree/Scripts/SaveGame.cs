...
namespace BayatGames.SaveGameFree
{
    ...
    public static class SaveGame
    {
        ...
        private static string m_EncodePassword = "dungeon_labs";
        ...
        public static string Save<T>(string identifier, T obj)
        {
            return Save<T>(identifier, obj, Encode, EncodePassword, Serializer, Encoder, DefaultEncoding, SavePath);
        }
        ...
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        public static string Save<T>(string identifier, T obj, bool encode)
        {
            return Save<T>(identifier, obj, encode, EncodePassword, Serializer, Encoder, DefaultEncoding, SavePath);
        }
        ...
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        public static string Save<T>(string identifier, T obj, bool encode, string password, ISaveGameSerializer serializer, ISaveGameEncoder encoder, Encoding encoding, SaveGamePath path)
        {
            ...
            if (!IsFilePath(identifier))
            {
                switch (path)
                {
                    ...
                    case SaveGamePath.PersistentDataPath:
                        filePath = string.Format("{0}/Save/{1}", Application.persistentDataPath, identifier);
                        break;
                    case SaveGamePath.DataPath:
                        filePath = string.Format("{0}/Save/{1}", Application.dataPath, identifier);
                        break;
                }
            }
            else
        ...
        public static T Load<T>(string identifier, T defaultValue, bool encode, string password, ISaveGameSerializer serializer, ISaveGameEncoder encoder, Encoding encoding, SaveGamePath path)
        {
...
#endif
            {
                Debug.LogWarningFormat(
                    ...
                    "returning the default(T) instance." + filePath,
                    filePath,
                    identifier);
                return result;
            }
            ...
            if (encode)
            {
                ...
                try
                {
                    decoded = encoder.Decode(data, password);
                }
