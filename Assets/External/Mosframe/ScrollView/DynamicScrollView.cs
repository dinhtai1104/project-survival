 ...
 */
 ...
 namespace Mosframe {
    ...
    public abstract class DynamicScrollView : UIBehaviour {
        ...
        public UnityEvent<int> onScroll         = null;
        ...
        public void scrollFixByItemIndex(int itemIndex)
        {
            var totalLen = this.contentSize;
            var itemLen = totalLen / this.totalItemCount;
            var pos = itemLen * itemIndex;
            ...
            {
                contentRect.anchoredPosition = new Vector2(-pos, 0);
            }
            else
            {
        ...
        public Tween scroll(int itemIndex, float time, TweenCallback callback, float offset = 0, Ease ease = Ease.OutQuad)
        {
            var totalLen = this.contentSize;
            var itemLen = itemSize;
            var pos = itemLen * itemIndex;
            var targetPos = pos;

            return contentRect.DOAnchorPosY(targetPos + offset, time).OnComplete(callback).SetId(gameObject).SetEase(ease);
        }

        public float GetPositionIndex(int itemIndex)
        {
            var totalLen = this.contentSize;
            var itemLen = totalLen / this.totalItemCount;
            var pos = itemLen * itemIndex;
        ...
        protected virtual void onSeedData() {
	    ...
	    public void Update () {
		    ...
		    while( this.contentAnchoredPosition - this.prevAnchoredPosition  < -this.itemSize * 2f) {
                ...
                onScroll?.Invoke(this.nextInsertItemNo);
            }

            // [ Scroll down ]

            while ( this.contentAnchoredPosition - this.prevAnchoredPosition > -this.itemSize) {
