...
namespace RVO
{
    ...
    internal class Agent
    {
        ...
        internal Vector2RVO position_;
        internal Vector2RVO prefVelocity_;
        internal Vector2RVO velocity_;
        internal int id_ = 0;
        ...
        private Vector2RVO newVelocity_;
        ...
        internal void computeNewVelocity()
        {
            ...
            for (int i = 0; i < obstacleNeighbors_.Count; ++i)
            {
                ...
                Vector2RVO relativePosition1 = obstacle1.point_ - position_;
                Vector2RVO relativePosition2 = obstacle2.point_ - position_;
                ...
                Vector2RVO obstacleVector = obstacle2.point_ - obstacle1.point_;
                ...
                if (s < 0.0f && distSq1 <= radiusSq)
                {
                    ...
                    {
                        line.point = new Vector2RVO(0.0f, 0.0f);
                        line.direction = RVOMath.normalize(new Vector2RVO(-relativePosition1.y(), relativePosition1.x()));
                        orcaLines_.Add(line);
                    }
                ...
                else if (s > 1.0f && distSq2 <= radiusSq)
                {
                    ...
                    {
                        line.point = new Vector2RVO(0.0f, 0.0f);
                        line.direction = RVOMath.normalize(new Vector2RVO(-relativePosition2.y(), relativePosition2.x()));
                        orcaLines_.Add(line);
                    }
                ...
                Vector2RVO leftLegDirection, rightLegDirection;
                ...
                if (s < 0.0f && distSqLine <= radiusSq)
                {
                    ...
                    leftLegDirection = new Vector2RVO(relativePosition1.x() * leg1 - relativePosition1.y() * radius_, relativePosition1.x() * radius_ + relativePosition1.y() * leg1) / distSq1;
                    rightLegDirection = new Vector2RVO(relativePosition1.x() * leg1 + relativePosition1.y() * radius_, -relativePosition1.x() * radius_ + relativePosition1.y() * leg1) / distSq1;
                }
                else if (s > 1.0f && distSqLine <= radiusSq)
                {
                    ...
                    leftLegDirection = new Vector2RVO(relativePosition2.x() * leg2 - relativePosition2.y() * radius_, relativePosition2.x() * radius_ + relativePosition2.y() * leg2) / distSq2;
                    rightLegDirection = new Vector2RVO(relativePosition2.x() * leg2 + relativePosition2.y() * radius_, -relativePosition2.x() * radius_ + relativePosition2.y() * leg2) / distSq2;
                }
                else
                {
                    /* Usual situation. */
                ...
                Vector2RVO leftCutOff = invTimeHorizonObst * (obstacle1.point_ - position_);
                Vector2RVO rightCutOff = invTimeHorizonObst * (obstacle2.point_ - position_);
                Vector2RVO cutOffVector = rightCutOff - leftCutOff;
                ...
                if ((t < 0.0f && tLeft < 0.0f) || (obstacle1 == obstacle2 && tLeft < 0.0f && tRight < 0.0f))
                {
                    ...
                    Vector2RVO unitW = RVOMath.normalize(velocity_ - leftCutOff);

                    line.direction = new Vector2RVO(unitW.y(), -unitW.x());
                ...
                else if (t > 1.0f && tRight < 0.0f)
                {
                    ...
                    Vector2RVO unitW = RVOMath.normalize(velocity_ - rightCutOff);
                ...
                if (distSqCutoff <= distSqLeft && distSqCutoff <= distSqRight)
                {
                    ...
                    line.direction = -obstacle1.direction_;
                    line.point = leftCutOff + radius_ * invTimeHorizonObst * new Vector2RVO(-line.direction.y(), line.direction.x());
                    orcaLines_.Add(line);
                ...
                line.point = rightCutOff + radius_ * invTimeHorizonObst * new Vector2RVO(-line.direction.y(), line.direction.x());
                orcaLines_.Add(line);
            }
            ...
            for (int i = 0; i < agentNeighbors_.Count; ++i)
            {
                ...
                Vector2RVO relativePosition = other.position_ - position_;
                Vector2RVO relativeVelocity = velocity_ - other.velocity_;
                ...
                {
                    /* No collision. */
                    Vector2RVO w = relativeVelocity - invTimeHorizon * relativePosition;
                    ...
                    else
                    {
                        ...
                        {
                            /* Project on left leg. */
                            line.direction = new Vector2RVO(relativePosition.x() * leg - relativePosition.y() * combinedRadius, relativePosition.x() * combinedRadius + relativePosition.y() * leg) / distSq;
                        }
                        else
                        {
                            /* Project on right leg. */
                ...
                else
                {
                    ...
                    /* Vector from cutoff center to relative velocity. */
                    Vector2RVO w = relativeVelocity - invTimeStep * relativePosition;
         ...
         * </param>
         */
        private bool linearProgram1(IList<Line> lines, int lineNo, float radius, Vector2RVO optVelocity, bool directionOpt, ref Vector2RVO result)
        {
         ...
         * </param>
         */
        private int linearProgram2(IList<Line> lines, float radius, Vector2RVO optVelocity, bool directionOpt, ref Vector2RVO result)
        {
            if (directionOpt)
            {
                /*
         ...
         * </param>
         */
        private void linearProgram3(IList<Line> lines, int numObstLines, int beginLine, float radius, ref Vector2RVO result)
        {
            float distance = 0.0f;
            ...
            for (int i = beginLine; i < lines.Count; ++i)
            {
                if (RVOMath.det(lines[i].direction, lines[i].point - result) > distance)
                {
                    ...
                    Vector2RVO tempResult = result;
                    if (linearProgram2(projLines, radius, new Vector2RVO(-lines[i].direction.y(), lines[i].direction.x()), true, ref result) < projLines.Count)
                    {
                        /*
