...
namespace RVO
{
    ...
    internal class KdTree
    {
         ...
         * tested.</param>
         */
        internal bool queryVisibility(Vector2RVO q1, Vector2RVO q2, float radius)
        {
            return queryVisibilityRecursive(q1, q2, radius, obstacleTree_);
        }

        internal int queryNearAgent(Vector2RVO point, float radius)
        {
...

            {
                ...
                for (int j = 0; j < obstacles.Count; ++j)
                {
                    ...
                    else
                    {
                        ...
                        Vector2RVO splitPoint = obstacleJ1.point_ + t * (obstacleJ2.point_ - obstacleJ1.point_);
        ...
        private void queryAgentTreeRecursive(Vector2RVO position, ref float rangeSq, ref int agentNo, int node)
        {
         ...
         * <param name="node">The current obstacle k-D node.</param>
         */
        private bool queryVisibilityRecursive(Vector2RVO q1, Vector2RVO q2, float radius, ObstacleTreeNode node)
        {
            if (node == null)
            {
                return true;
