...
namespace Lean.Pool
{
	...
	public class LeanGameObjectPool : MonoBehaviour, ISerializationCallbackReceiver
	{
		...
		public bool Recycle { set { recycle = value; } get { return recycle; } } [SerializeField] private bool recycle = true;
		...
		public void Clean()
		{
			for (var i = spawnedClonesList.Count - 1; i >= 0; i--)
			{
				DestroyImmediate(spawnedClonesList[i].gameObject);
			}
			for (var i = despawnedClones.Count - 1; i >= 0; i--)
			{
				DestroyImmediate(despawnedClones[i].gameObject);
			}
