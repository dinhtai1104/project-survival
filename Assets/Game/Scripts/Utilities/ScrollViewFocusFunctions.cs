...
public static class ScrollViewFocusFunctions
{
	...
	private static IEnumerator MoveToScrollPositionCoroutine(this ScrollRect scrollView, Vector2 targetNormalizedPos, float speed)
	{
		Vector2 initialNormalizedPos = scrollView.normalizedPosition;

		
		while (Vector2.Distance(scrollView.normalizedPosition,targetNormalizedPos)>0.05f)
		{
			scrollView.normalizedPosition = Vector2.MoveTowards(initialNormalizedPos, targetNormalizedPos, speed);
			yield return null;
		}
	...
	public static IEnumerator FocusOnItemCoroutine(this ScrollRect scrollView, RectTransform item, float speed, float delay = 0 )
	{
		yield return new WaitForSeconds(delay);
		yield return scrollView.LerpToScrollPositionCoroutine( scrollView.CalculateFocusedScrollPosition( item ), speed );
	}
	public static IEnumerator MoveToItemCoroutine(this ScrollRect scrollView, RectTransform item, float speed)
	{
		yield return scrollView.MoveToScrollPositionCoroutine(scrollView.CalculateFocusedScrollPosition(item), speed);
	}
}